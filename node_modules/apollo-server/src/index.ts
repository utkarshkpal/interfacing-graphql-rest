...
export class ApolloServer extends ApolloServerBase {
  ...
  private createServerInfo(server : http.Server, subscriptionsPath?: string,) : ServerInfo {
    ...
    // corresponding loopback ip. Note that the url field we're setting is primarily
    // for consumption by our test suite. If this heuristic is wrong for your use
    // case, explicitly specify a frontend host (in the `frontends.host` field in
    // your engine config, or in the `host` option to ApolloServer.listen).
    let hostForUrl = serverInfo.address;
    if (serverInfo.address === '' || serverInfo.address === '::') 
      hostForUrl = 'localhost';
    
    serverInfo.url = require('url').format({protocol: 'http', hostname: hostForUrl, port: serverInfo.port, pathname: this.graphqlPath});

    serverInfo.subscriptionsUrl = require('url').format({protocol: 'ws', hostname: hostForUrl, port: serverInfo.port, slashes: true, pathname: subscriptionsPath});

    return serverInfo;
  }

  public applyMiddleware() {
    throw new Error('To use Apollo Server with an existing express application, please use apollo-ser' +
        'ver-express',);
  }
  ...
  public async listen(...opts : Array < any >) : Promise < ServerInfo > {
    ...
    await new Promise(resolve => {
      ...
      // If the user passed a callback to listen, it'll get called in addition to our
      // resolver. They won't have the ability to get the ServerInfo object unless
      // they use our Promise, though.
      httpServer.listen(...(opts.length
        ? opts
        : [
          {
            port: 4001
